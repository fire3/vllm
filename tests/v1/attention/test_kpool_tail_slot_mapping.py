...
def test_triton_mapping_matches_cpu(per_req, num_actual, padded_len):
    ...
    the last request). Padding beyond num_actual_tokens is normalized to the
    -1 sentinel in both paths, so a reused batch buffer cannot leak stale slot
    ids into the tail kernels."""
    positions, qsl, slot_mapping, _, num_reqs = make_batch(
        per_req, padded_len=padded_len
    )
    # Replace the all--1 placeholder slots with sentinel values: the padding
    # range must come back as the -1 sentinel, not as these ids.
    ...
    assert (ref[num_actual:] == -1).all()
    assert (got.cpu()[num_actual:] == -1).all()
