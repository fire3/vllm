...
- 本地 wheel 版本由 git tag 推导（无需 setuptools_scm，按 setuptools-scm
  的下一版本规则取公开版本），默认追加 `+sm89` local 标记（如
  `0.27.1.dev28+g7a1d4cf79` → `vllm-0.27.1+sm89-...`）；可用
  `VLLM_VERSION_OVERRIDE` 覆盖，如 `0.27.0+cu130sm89`。
