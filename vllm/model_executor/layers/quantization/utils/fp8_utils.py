...
def w8a8_triton_block_scaled_mm(
    ...
    # Triton cannot bind E8M0 scale tensors directly on any platform, so
    # decode them to fp32 before launching the kernel. Checkpoints with
    # exponent-only UE8M0 scales (e.g. DeepSeek-V4) are decoded here.
    if As.dtype == torch.float8_e8m0fnu:
