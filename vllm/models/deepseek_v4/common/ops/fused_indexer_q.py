...
class FusedIndexerQRopeQuantTritonKernel(
    ...
    def kernel(
        ...
        # `weights` to apply per-token Q scale inline. `scale_combined` is
        # precomputed on the host as ``float(softmax_scale * head_scale)`` so the
        # multiply order ``(w * scale_combined) * q_scale`` is bit-identical to
        # the CuteDSL reference kernel (fused_indexer_q_cutedsl.py). See the MXFP4
        # kernel below for contrasting convention (scales live with the Q values,
    ...
    def warmup_inputs(self, compile_key: CompileKey) -> dict[str, Any]:
        ...
        return dict(
            ...
            index_weights_scale_combined=1.0,
