...
"""Triton FP8 MQA logits fallbacks for environments without DeepGEMM.
...
It also carries the SM89/Ada sparse-indexer fallback (``fp8_mqa_logits_triton``
for prefill and ``fp8_paged_mqa_logits_triton`` for decode), ported from the
v0.28.0-sm89 branch. Both preserve the DeepGEMM reference semantics:
...
@triton.jit
def _fp8_mqa_logits_sm89_kernel(
    Q_ptr,  # fp8e4m3 [seq_len, H, D]
    KV_ptr,  # fp8e4m3 [seq_len_kv, D]
    kv_scales_ptr,  # fp32 [seq_len_kv]
    weights_ptr,  # fp32 [seq_len, H]
    ...
    for _ in tl.range(0, shifted_unmasked_end, BLOCK_KV):
        ...
        # the MMA accumulates in fp32 through fp16 tensor cores. This matches
        # the DeepGEMM reference op order on SM90/SM100/SM120:
        # logit = (sum_h relu(q.k_h) * w_h) * s_kv.
        scores = tl.dot(
...
def fp8_mqa_logits_triton(
    ...
    _fp8_mqa_logits_sm89_kernel[(seq_len,)](
        Q_ptr=q,
        KV_ptr=k_fp8,
        kv_scales_ptr=kv_scales_1d,
        weights_ptr=weights,
...
def fp8_paged_mqa_logits_triton(
    ...
    *,
    ...
    Args:
        ...
        max_context_len: Host-side upper bound on the effective context length
            in this batch (same units as ``context_lens``). When omitted the
            launcher derives it from the device tensor, which adds a D2H sync.
    """
