...
def _kpool_tail_slot_mapping_kernel(
    ...
    if pid < num_reqs:
        ...
        # Padded rows beyond the actual tokens: emit the negative sentinel the
        # tail kernels guard on. The reused batch slot_mapping can still hold
        # valid slot ids from a previous, larger batch, and the kpool indexer's
        # tail-seed kernel iterates this padded range and indexes the tail cache
        # with these slots, so a stale id there corrupts (or faults) the tail.
        offs = num_actual_tokens + (pid - num_reqs) * BLOCK + tl.arange(0, BLOCK)
        tl.store(out_ptr + offs, -1, mask=offs < num_tokens)
...
def compute_kpool_tail_slot_mapping(
    ...
    # Same padded-row normalization as the Triton tail branch above, so both
    # paths agree (see _kpool_tail_slot_mapping_kernel).
